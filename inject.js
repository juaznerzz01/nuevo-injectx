...
const MODES = {
    ...
    const PRICE_WINDOW_SIZE = 32;
    const MOVE_WINDOW_SIZE = 32;
   ...
   let fxProbEl = null;
    ...
    const buttonsRow = document.createElement('div');
        ...
        if (!cleaned.includes('.')) return null; // preferimos decimales
        const num = parseFloat(cleaned);
        if (Number.isNaN(num)) return null;
        if (num <= 0) return null;
        const decimals = (cleaned.split('.')[1] || '').length;
        if (decimals < 3) return null;
        if (num > 20 || num < 0.0001) return null; // evita payout/balance
        return num;
    }
        ...
        const selectorCandidates = [
            ...
            for (const el of candidates) {
                const candidate = parsePriceText(el.textContent || '');
                if (candidate !== null) {
                    return { el, selector: `${containerSel} span/div` };
                }
            }
        }
    ...
    function triggerAutoTrade(side) {
       ...
       function calculate() {
        if (lastObservedPrice === null) return;
