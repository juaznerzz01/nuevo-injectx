...
const MODES = {
    ...
    const PRICE_WINDOW_SIZE = 32; // ~7-8s con intervalos de 250ms
    const MOVE_WINDOW_SIZE = 32;  // ~7-8s para slope micro
  ...
  '.speedometer',
  // '.speedometer-container', // NO ocultar: aquí vive el botón TURN ON / SELLER VOLUME
  '.gauge',
  '.percentage',
  '.percentage-wrapper'
];
...
#fx-vision-overlay .speedometer-container,
...
#fx-vision-overlay .percentage-wrapper {
    ...
    const buttonsRow = document.createElement('div');
        ...
        const hasMoneyKeyword = /(payout|balance|investment|deposit|withdrawal|your payout)/i.test(lowered);
        const hasCurrencySymbol = /[$€£₹¥]/.test(text);
        if (hasMoneyKeyword || hasCurrencySymbol) return null;

        const forbiddenContainers = [
            '.trade-panel', '.deal-panel', '.right-panel', '.sidebar', '.account',
            '.balance', '.payout', '.investment', '.withdrawal', '.deposit', '.right-column'
        ];
        if (el) {
            const inForbidden = forbiddenContainers.some(sel => el.closest(sel));
            if (inForbidden) return null;
        }

        const cleaned = text.replace(/[^0-9.,-]/g, '').replace(',', '.');
        if (!cleaned || cleaned.length < 2) return null;
        if (!cleaned.includes('.')) return null; // requerimos decimales

        const decimals = (cleaned.split('.')[1] || '').length;
        if (decimals < 2 || decimals > 6) return null;
        const num = parseFloat(cleaned);
        if (Number.isNaN(num)) return null;
        if (num <= 0 || num > 200000) return null;
        if (num < 0.00001) return null;
        return num;
    }
        ...
        const scoreCandidate = (el, selectorLabel) => {
            const parsed = parsePriceText(el.textContent || '', el);
            if (parsed === null) return;

            const style = window.getComputedStyle ? getComputedStyle(el) : null;
            const visible = style ? (style.display !== 'none' && style.visibility !== 'hidden' && parseFloat(style.opacity || '1') > 0) : true;
            const decimals = ((el.textContent || '').replace(/[^0-9.,]/g, '').replace(',', '.').split('.')[1] || '').length;
            const nearChart = !!el.closest(chartContainers.join(','));

            let score = 0;
            if (visible) score += 3;
            if (nearChart) score += 3;
            if (decimals >= 3 && decimals <= 5) score += 2; else score += 1;
            if (el.isConnected) score += 1;

            if (!best || score > best.score) {
                best = { el, selector: selectorLabel, score, parsed };
            }
        };

        for (const selector of selectorCandidates) {
            const els = document.querySelectorAll(selector);
            els.forEach(el => scoreCandidate(el, selector));
            if (best && best.selector === selector) break;
        }

        if (!best) {
            for (const containerSel of chartContainers) {
                const container = document.querySelector(containerSel);
                if (!container) continue;
                const candidates = Array.from(container.querySelectorAll('span, div')).slice(0, 60);
                for (const el of candidates) {
                    scoreCandidate(el, `${containerSel} span/div`);
                }
            }
        }

return best ? { el: best.el, selector: best.selector } : null;
    }

                function setupPriceObserver() {
        const found = findPriceElement();
       if (!found) {
            lastPriceSelector = null;
            return null;
        }

                 const { el, selector } = found;
        lastPriceSelector = selector;

                const observePrice = () => {
            const candidate = parsePriceText(el.textContent || '', el);
            if (candidate !== null && candidate !== lastObservedPrice) {
                lastObservedPrice = candidate;
                handlePriceTick(candidate);
            }
             };

            observePrice(); // inicial

        const observer = new MutationObserver(() => observePrice());
    ...
    function determineMarketStatus(now, isChop, medianAbsDelta, recentRange) {
        if (lastObservedPrice === null || (now - lastPriceChangeAt > 2000)) {
            return "NO PRICE";
        }
    ...
    function triggerAutoTrade(side) {
       ...
       ensureFxSignalBox();
        ...
        // === PROBABILIDAD BRUTA POR RATIO DE TICKS (ventana corta) ===
        ...
        if (probHistory.length > PROB_WINDOW) probHistory.shift();
         ...
         let smoothedProbUp = probHistory.reduce((a, b) => a + b, 0) / probHistory.length;
        let smoothedProbDown = 100 - smoothedProbUp;

        // Validación de coherencia macro vs probabilidad
        let macroCoherencePenalty = 0;

        const avgVol = volHistory.length > 0
            ? volHistory.reduce((a, b) => a + b, 0) / volHistory.length
            : 0;
        const dynamicMinFreq = Math.max(MIN_FREQ, Math.round((freq || 1) * 0.6 + (avgVol * 1000)));

        // === TENDENCIA MICRO + MACRO (EMA) ===
        const emaFast = calcEMA(priceWindow, EMA_FAST);
        const emaSlow = calcEMA(priceWindow, EMA_SLOW);
        let macroTrend = "none";
       ...
       const recentPrices = priceWindow.slice(-30);
        ...
        const slopeThreshold = recentRange * 0.015;


        const slopeUp = microSlope >= slopeThreshold;
        const slopeDown = microSlope <= -slopeThreshold;

        // === RANGO Y PULLBACK ===
        const deltas = [];
        for (let i = 1; i < priceWindow.length; i++) {
            deltas.push(priceWindow[i] - priceWindow[i - 1]);
        }
        const absDeltas = deltas.map(Math.abs);
        const medianAbsDelta = median(absDeltas);
        const minRange = medianAbsDelta * RANGE_MULTIPLIER;
        const isChop = recentRange > 0 ? recentRange < minRange : true;
        const isPullback = medianAbsDelta === 0
            ? Math.abs(lastDelta) < 0.0005
            : Math.abs(lastDelta) <= medianAbsDelta * 1.2;
        ...
        const hasDirection = (macroTrend === "up" && slopeUp) || (macroTrend === "down" && slopeDown);

        if (macroTrend === "down" && smoothedProbUp > 60) {
            macroCoherencePenalty = 8;
            smoothedProbUp = Math.max(MIN_PROB - 2, smoothedProbUp - macroCoherencePenalty);
            smoothedProbDown = 100 - smoothedProbUp;
        } else if (macroTrend === "up" && smoothedProbDown > 60) {
            macroCoherencePenalty = 8;
            smoothedProbDown = Math.max(MIN_PROB - 2, smoothedProbDown - macroCoherencePenalty);
            smoothedProbUp = 100 - smoothedProbDown;
        }


        if (hasVolatility && !isChop && hasDirection && isPullback) {
            // Tendencia combinada: macro + micro + prob simétrica
            if (macroTrend === "up" && slopeUp && smoothedProbUp >= MIN_PROB) {
                candidateSide = "CALL";
          } else if (macroTrend === "down" && slopeDown && smoothedProbDown >= MIN_PROB) {
                candidateSide = "PUT";
        ...
        const allSameSide =
            ...
            sideHistory.every(s => s === candidateSide && s !== "NO TRADE");
             ...
             const withinProbCeiling =
            (candidateSide === "CALL" && smoothedProbUp <= 80) ||
            (candidateSide === "PUT" && smoothedProbDown <= 80);

        const isStrongCandidate =
            allSameSide &&
            (candidateSide === "CALL" || candidateSide === "PUT") &&
            ((candidateSide === "CALL" && smoothedProbUp >= STRONG_PROB) ||
             (candidateSide === "PUT" && smoothedProbDown >= STRONG_PROB)) &&
            hasVolatility &&
            !isChop &&
            isPullback &&
            withinProbCeiling;
        ...
        const isCandidate = (candidateSide === "CALL" || candidateSide === "PUT");
        ...
        // Filtro de rango/congestión adicional
        let avgTick = 0;
        if (recentPrices.length > 1) {

            const diffs = [];
            for (let i = 1; i < recentPrices.length; i++) {
                diffs.push(Math.abs(recentPrices[i] - recentPrices[i - 1]));
            }
             avgTick = diffs.reduce((a, b) => a + b, 0) / diffs.length;
        }

        const isRangeCompressed = recentRange > 0 && avgTick > 0 && recentRange < (avgTick * 2.5);
        ...
        if (currentSignalSide === "CALL" || currentSignalSide === "PUT") {
            ...
            if (isChop) {
                ...
                stableSignal = "NO TRADE";
                stateLabel = "WAIT PULLBACK";
            } else if (elapsed >= MIN_SIGNAL_DURATION && !isRangeCompressed && isStrongCandidate && withinProbCeiling) {
                stableSignal = currentSignalSide;
                stateLabel = currentSignalSide === "CALL"
                    ? "ENTRY WINDOW UP"
                    : "ENTRY WINDOW DOWN";
                    } else if (elapsed >= MIN_SIGNAL_DURATION && isRangeCompressed) {
                stableSignal = "NO TRADE";
                stateLabel = "RANGE / NO TRADE";
            } else {
                stateLabel = "PRE-SIGNAL";
            }
             } else {
                stateLabel = isChop ? "CHOP / NO TRADE" : "NO CLEAR SIGNAL";
        }
         if (marketStatus !== "OK") {
            stableSignal = "NO TRADE";
        ...
        if (fxFreqEl) {
            fxFreqEl.textContent = `Freq: ${freq} | ${tickRate.toFixed(2)} t/s (min ${dynamicMinFreq})`;
        }
        ...
        if (fxDebugEl) {
            ...
            fxDebugEl.textContent = `Price: ${priceStr} | d: ${deltaStr} | probUp: ${smoothedProbUp.toFixed(1)} | range: ${recentRange.toFixed(5)} | src: ${lastPriceSelector || 'n/a'}`;
        }
        updateMarketUI();
        updateAutoTradeButton();
        ...
        if (fxCallTextEl && fxPutTextEl) {
            ...
            const isEntryWindow =
                stateLabel.startsWith("ENTRY WINDOW") ||
                stateLabel.startsWith("PRE-SIGNAL");

        if (isEntryWindow) {
                if (displaySignal === "CALL") {
                    fxCallTextEl.style.display = "block";
                } else if (displaySignal === "PUT") {
                    fxPutTextEl.style.display = "block";
                }
            }
        }
         const entryWindowActive = stateLabel.startsWith("ENTRY WINDOW");
     ...
     // Observador de precio en vivo
    priceObserver = setupPriceObserver();
    setInterval(() => {
        if (!priceObserver) {
            priceObserver = setupPriceObserver();
        }
