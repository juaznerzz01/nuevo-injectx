...
const MODES = {
    AGGRESSIVE: {
        MIN_PROB: 48,     // antes 55 → ahora entra mucho más
        STRONG_PROB: 55,  // antes 60+
        MIN_FREQ: 1,
        EMA_FAST: 8,
        EMA_SLOW: 34,
        MIN_SIGNAL_MS: 900,   // ~0.8s para confirmar
   ...
   let fxProbEl = null;
    ...
    function sendPanelSnapshotToN8n(probRaw, trend, signal, stateLabel, freq) {
       ...
       function calculate() {
        ...
        const probUpRaw = total > 0 ? (upTicks / total) * 100 : 0;
        ...
        const slopeWindow = 40;
        const recentSlopePrices = priceHistory.slice(-slopeWindow);
        let slopeThreshold = 0;
        if (recentSlopePrices.length > 1) {
            const recentMax = Math.max(...recentSlopePrices);
            const recentMin = Math.min(...recentSlopePrices);
            const recentRange = recentMax - recentMin;
            const avgRangePerTick = recentRange / recentSlopePrices.length;
            slopeThreshold = avgRangePerTick > 0 ? avgRangePerTick * 0.04 : 0;
        }

        const slopeUp = slopeThreshold > 0 ? microSlope >= slopeThreshold : microSlope > 0;
        const slopeDown = slopeThreshold > 0 ? microSlope <= -slopeThreshold : microSlope < 0;
        ...
        const recentPrices = priceHistory.slice(-rangeWindow);
        let recentRange = 0;
        let minRange = 0;
        if (recentPrices.length > 1) {
            ...
            const avgTick = diffs.reduce((a, b) => a + b, 0) / diffs.length;
            minRange = avgTick * 3;
        }

        const isRangeCompressed = recentRange > 0 && minRange > 0 && recentRange < minRange;


        if (currentSignalSide === "CALL" || currentSignalSide === "PUT") {
            const elapsed = now - signalStartAt;

            if (elapsed >= MIN_SIGNAL_DURATION && !isRangeCompressed) {
