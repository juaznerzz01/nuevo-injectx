...
const MODES = {
    ...
    const CANVAS_TICK_THROTTLE_MS = 100;
    const PRICE_REGEX = /\b\d{2,6}\.\d{2,6}\b/;
    // Estado de análisis
      window.lastObservedPrice = null;
    window.priceSource = 'CANVAS';
    let lastObservedPrice = null;
    let lastPriceSelector = 'CANVAS';
    let lastWsPriceAt = 0;
...
#fx-vision-overlay .percentage-wrapper {
    ...
    function resetPriceBuffers() {
        ...
        window.lastObservedPrice = null;
        window.priceSource = 'CANVAS';
        lastPriceSelector = 'CANVAS';
        lastWsPriceAt = 0;
        lastCanvasCaptureAt = 0;
        lastPriceChangeAt = 0;
    }

    // Hook temprano de Canvas para capturar precios renderizados en Pixi/WebGL
            ...
            try {
                if (typeof text === 'string') {
                    ...
                    if (
                        trimmed &&
                        !hasNoise &&
                        now - lastCanvasCaptureAt >= CANVAS_TICK_THROTTLE_MS
                    ) {
                        const match = PRICE_REGEX.exec(trimmed);
                        if (match) {
                            const price = parseFloat(match[0]);
                            if (Number.isFinite(price)) {
                                lastCanvasCaptureAt = now;
                                window.__FX_PRICE__ = price;
                ...
                // Ignorar errores silenciosamente para no romper el render
            }
    ...
    // Determina el estado del mercado (Canvas es la fuente única)
    ...
    function triggerAutoTrade(side) {
     ...
     const priceChanged = lastPrice !== null && price !== lastPrice;
       ...
       ensureFxSignalBox();
        ...
        const isNewPriceTick = lastObservedPrice === null || price !== lastObservedPrice;
   ...
   if (isNewPriceTick) {
       ...
       const minRange = medianAbsDelta > 0 ? medianAbsDelta * RANGE_MULTIPLIER : 0;
        ...
        if (fxDebugEl) {
            const priceStr = Number.isFinite(price) ? price.toFixed(5) : '...';
            const deltaStr = lastDelta ? lastDelta.toFixed(5) : '0.00000';
            const sourceLabel = window.priceSource || lastPriceSelector || 'CANVAS';
            fxDebugEl.textContent = `Price: ${priceStr} | d: ${deltaStr} | src: ${sourceLabel}`;
        }
        updateMarketUI();
        updateAutoTradeButton();
